...
class WelcomePage(root.Frame):
    def __init__(self, parent, controller):
        ...
        button1 = root.Button(
            self, text="Add Food Items", command=lambda: controller.show_frame(addItem)
        )

        button2 = root.Button(
            self, text="Order Food", command=lambda: controller.show_frame(orderFood)
        )
...
class addItem(root.Frame):
    def __init__(self, parent, controller):
        ...
        addItemBtn = root.Button(
            self,
            text="Add Item",
            command=lambda: self.addFood(
                foodNameEntry.get(), foodPriceEntry.get(), infoLabel
            ),
        )
        ...
        goBackBtn = root.Button(
            self, text="<--", command=lambda: controller.show_frame(WelcomePage)
        )
